...
window.addEventListener("keydown", ({ keyCode }) => {
  // Press A
  if (keyCode === 76) return playA();

  // Press B
  if (keyCode === 65) return playB();

  // Press C
  if (keyCode === 75) return playC();

  // Press D
  if (keyCode === 83) return playD();

  // Press E
  if (keyCode === 72) return playE();

  // Press F
  if (keyCode === 77) return playF();

  // Press G
  if (keyCode === 73) return playG();
